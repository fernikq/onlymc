...
public class GuildAllianceCreateCommand extends CustomCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if(args.length < 3) {
            return ChatUtil.sendMessage(sender, MessagesManager.usage("/ga sojusz <tag> <tag>"));
        }
        String tag1 = args[1];
        String tag2 = args[2];
        this.plugin.getGuildManager().getGuildByTag(tag1).peek(guild1 -> {
            ...
            if(this.plugin.getAllianceManager().hasAlliance(guild1, guild2)){
                ChatUtil.sendMessage(sender, MessagesManager.error("Podane gildie maja juz sojusz!"));
                return;
            }
            this.plugin.getAllianceManager().createAlliance(guild1, guild2);
            ChatUtil.sendMessage(sender, "&8>> {n}Zawiazales sojusz pomiedzy gildia &8[&f"+guild1.getTag()+"&8] {n}a gildia &8[&f"+guild2.getTag()+"&8]");
