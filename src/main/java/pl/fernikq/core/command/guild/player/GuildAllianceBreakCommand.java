...
public class GuildAllianceBreakCommand extends CustomCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if(args.length < 2) {
            return ChatUtil.sendMessage(sender, MessagesManager.usage("/g zerwij <tag>"));
        }
        ...
        this.plugin.getUserManager().getUser(player.getUniqueId()).peek(user -> {
            ...
            if(targetGuild.equals(guild)){
                ChatUtil.sendMessage(sender, MessagesManager.error("Nie mozesz zerwac sojuszu z wlasna gildia!"));
                return;
            }
            if(!this.plugin.getAllianceManager().hasAlliance(guild, targetGuild)){
                ChatUtil.sendMessage(sender, MessagesManager.error("Twoja gildia nie posiada sojuszu z podana gildia!"));
                return;
            }
            this.plugin.getAllianceManager().removeAlliance(guild, targetGuild);
            String message = MessagesManager.guildAllianceBreakMessage;
