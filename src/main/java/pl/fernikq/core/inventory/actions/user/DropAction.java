...
public class DropAction implements InventoryAction {
    ...
    public void execute(Player player, Inventory inventory, int slot, ItemStack itemStack, InventoryClickEvent event) {
        ...
        if(dropActionType.equals(DropActionType.OPEN_BOSS_DROP)){
            this.plugin.getUserInventory().dropBoss(user).openInventory(player);
            return;
        }
        ...
        if(dropActionType.equals(DropActionType.OPEN_TURBO_SYSTEM)){
            this.plugin.getUserInventory().dropTurboSystem(user).openInventory(player);
            return;
        }
