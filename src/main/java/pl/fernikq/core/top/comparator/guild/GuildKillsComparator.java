...
public class GuildKillsComparator implements Sortable<Guild> {
    ...
    public GuildKillsComparator(CorePlugin plugin, TopType topType, TopKind topKind){
        this.plugin = plugin;
        ...
        this.guildComparator = new Comparator<Guild>() {
            ...
            public int compare(Guild o1, Guild o2) {
                int i = Integer.compare(o2.getKills(), o1.getKills());
                if(i == 0){
                    if(o1.getName() == null){
                        return -1;
                    }
    ...
    public InventoryGUI getInventory(Guild object) {
        ...
        for(int i = 0; i < 45; i++){
            ...
            inventoryGUI.addItem(new ItemBuilder(Material.ENDER_PORTAL_FRAME).setName(ChatUtil.fixColor("{n}Pozycja {c}"+(i + 1))).setLore(ChatUtil.fixColor(Arrays.asList(" ", "&8>> {n}Gildia&8: {c}"+topGuild.getTag(), "&8>> {n}Zabojstw&8: {c}"+topGuild.getKills()))).toItemStack());
        }
        inventoryGUI.setItem(49, new ItemBuilder(Material.ENDER_PORTAL_FRAME).setName(ChatUtil.fixColor("{n}Pozycja twojej gildii&8: {c}"+(object == null ? "Brak" : (getPositionByObject(object)+1))))
                .setLore(ChatUtil.fixColor(Arrays.asList(" ", "&8>> {n}Zabojstw&8: {c}"+((object == null ? "Brak" : object.getKills()))))).toItemStack());
        return inventoryGUI;
    }
