...
public class CoinsComparator implements Sortable<User> {
    ...
    public InventoryGUI getInventory(User object) {
        ...
        for(int i = 0; i < 45; i++){
            ...
            inventoryGUI.addItem(new ItemBuilder(Material.SKULL_ITEM).setDurability((short) 3).setName(ChatUtil.fixColor("{n}Pozycja {c}"+(i + 1))).setSkullOwner(topUser.getName()).setLore(ChatUtil.fixColor(Arrays.asList(" ", "&8>> {n}Gracz&8: {c}"+topUser.getName(), "&8>> {n}Posiada {c}"+topUser.getUserStat().getCoins()+" {n}monet"))).toItemStack());
        }
        inventoryGUI.setItem(49, new ItemBuilder(Material.SKULL_ITEM).setDurability((short) 3).setName(ChatUtil.fixColor("{n}Twoja pozycja&8: {c}"+(getPositionByObject(object)+1))).setSkullOwner(object.getName())
                .setLore(ChatUtil.fixColor(Arrays.asList(" ", "&8>> {n}Posiadasz {c}"+object.getUserStat().getCoins()+" {n}monet"))).toItemStack());
        return inventoryGUI;
    }
