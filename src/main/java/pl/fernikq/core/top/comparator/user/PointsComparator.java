...
public class PointsComparator implements Sortable<User> {
    ...
    public PointsComparator(CorePlugin plugin, TopType topType, TopKind topKind){
        this.plugin = plugin;
        ...
        this.userComparator = new Comparator<User>() {
            ...
            public int compare(User o1, User o2) {
                int i = Integer.compare(o2.getUserStat().getPoints(), o1.getUserStat().getPoints());
                if(i == 0){
                    if(o1.getName() == null){
                        return -1;
                    }
    ...
    public InventoryGUI getInventory(User object) {
        ...
        for(int i = 0; i < 45; i++){
            ...
            inventoryGUI.addItem(new ItemBuilder(Material.SKULL_ITEM).setDurability((short) 3).setName(ChatUtil.fixColor("{n}Pozycja {c}"+(i + 1))).setSkullOwner(topUser.getName()).setLore(ChatUtil.fixColor(Arrays.asList(" ", "&8>> {n}Gracz&8: {c}"+topUser.getName(), "&8>> {n}Posiada {c}"+topUser.getUserStat().getPoints()+" {n}punktow"))).toItemStack());
        }
        inventoryGUI.setItem(49, new ItemBuilder(Material.SKULL_ITEM).setDurability((short) 3).setName(ChatUtil.fixColor("{n}Twoja pozycja&8: {c}"+(getPositionByObject(object)+1))).setSkullOwner(object.getName())
                .setLore(ChatUtil.fixColor(Arrays.asList(" ", "&8>> {n}Posiadasz {c}"+object.getUserStat().getPoints()+" {n}punktow"))).toItemStack());
        return inventoryGUI;
    }
